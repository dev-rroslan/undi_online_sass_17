defmodule UndiOnlineWeb.CoreComponents do
  @moduledoc """
        ...
        <div class="flex items-center justify-center min-h-full">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="relative hidden bg-white shadow-lg rounded-2xl p-14 shadow-zinc-700/10 ring-1 ring-zinc-700/10 transition"
            >
              ...
              <div id={"#{@id}-content"}>
                ...
                <div :if={@confirm != [] or @cancel != []} class="flex items-center mb-4 ml-6 gap-5">
                  <.button
                    :for={confirm <- @confirm}
                    id={"#{@id}-confirm"}
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed hidden top-2 right-2 w-80 sm:w-96 z-50 alert shadow-md",
        @kind == :info && "alert-info",
        @kind == :error && "alert-error",
        @id == "disconnected" && "transition delay-700"
      ]}
      {@rest}
    >
  ...
  def input(%{type: "checkbox"} = assigns) do
    ...
    ~H"""
    <label phx-feedback-for={@name} class="flex items-center cursor-pointer label gap-2">
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="label label-text">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def error(assigns) do
    ~H"""
    <p class="flex mt-3 text-sm phx-no-feedback:hidden gap-3 leading-6 text-error">
  ...
  def table(assigns) do
    ~H"""
    <div id={@id} class="overflow-x-auto sm:overflow-visible">
      <table class="table w-full">
        ...
        <tbody>
          <tr
            ...
            class="relative group hover:bg-base-200/50"
          >
            <td
              :for={{col, i} <- Enum.with_index(@col)}
              phx-click={@row_click && @row_click.(row)}
              class={["text-sm group-hover:bg-base-200/50", @row_click && "hover:cursor-pointer"]}
            >
              <span class={["relative", i == 0 && "font-semibold text-base-content/90"]}>
                <%= render_slot(col, row) %>
              </span>
            </td>
            <td :if={@action != []} div class="flex items-center text-sm text-right group-hover:bg-base-200/50 whitespace-nowrap gap-2">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(UndiOnlineWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(UndiOnlineWeb.Gettext, "errors", msg, opts)
    end
  end
