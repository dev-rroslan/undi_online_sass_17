defmodule UndiOnlineWeb.UserAuth do
  @moduledoc """
  Plugs and functions regarding user authentication
  """
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_undi_online_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      UndiOnlineWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule UndiOnlineWeb.PageLive do
        ...
        on_mount {UndiOnlineWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{UndiOnlineWeb.UserAuth, :ensure_authenticated}] do
        live "/profile", ProfileLive, :index
      end
  """
  def on_mount(:mount_current_user, params, session, socket) do
    {:cont, mount_current_user(params, session, socket)}
  end

  def on_mount(:ensure_authenticated, params, session, socket) do
    socket = mount_current_user(params, session, socket)
  ...
  defp mount_current_user(params, session, socket) do
    case session do
  ...
  defp assign_new_current_user_and_account(params, session, socket, user_token) do
    socket =
      socket
